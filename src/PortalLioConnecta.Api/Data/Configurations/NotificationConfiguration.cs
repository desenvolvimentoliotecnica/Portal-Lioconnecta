...
public class NotificationConfiguration : IEntityTypeConfiguration<Notification>
{
    public void Configure(EntityTypeBuilder<Notification> builder)
    {
        ...
        builder.HasIndex(item => item.RecipientPortalUserId);

        builder.HasOne(item => item.RecipientPortalUser)
            .WithMany()
            .HasForeignKey(item => item.RecipientPortalUserId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasOne(item => item.ActorPortalUser)
            .WithMany()
            .HasForeignKey(item => item.ActorPortalUserId)
