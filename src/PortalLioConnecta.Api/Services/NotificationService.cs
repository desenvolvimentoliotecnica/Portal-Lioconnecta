...
public class NotificationService : INotificationService
{
    ...
    public NotificationService(PortalLioConnectaDbContext dbContext)
    {
    ...
    public async Task<NotificationListResponse> GetForUserAsync(Guid portalUserId, CancellationToken cancellationToken)
    {
        ...
        var items = await _dbContext.Notifications
            ...
            .Where(item => item.RecipientPortalUserId == null || item.RecipientPortalUserId == portalUserId)
        ...
        var exists = await _dbContext.Notifications
            .AnyAsync(
                item => item.Id == notificationId &&
                        item.IsActive &&
                        (item.RecipientPortalUserId == null || item.RecipientPortalUserId == portalUserId),
                cancellationToken);
