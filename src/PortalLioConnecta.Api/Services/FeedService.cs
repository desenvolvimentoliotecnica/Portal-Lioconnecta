...
public class FeedService : IFeedService
{
    ...
    private readonly IFeedInteractionNotificationService _feedInteractionNotificationService;

    public FeedService(
        PortalLioConnectaDbContext dbContext,
        ICommunicationService communicationService,
        IFeedInteractionNotificationService feedInteractionNotificationService)
    {
        _dbContext = dbContext;
        _communicationService = communicationService;
        _feedInteractionNotificationService = feedInteractionNotificationService;
    }
    ...
    public async Task<FeedResponse> GetFeedAsync(Guid? portalUserId, CancellationToken cancellationToken)
    {
        ...
        items.AddRange(await BuildShareFeedItemsAsync(
            portalUserId,
            userPostLikeCounts,
            userPostShareCounts,
            likedUserPostIds,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await _feedInteractionNotificationService.RegisterCommentAsync(
            feedPost.PortalUserId,
            portalUserId,
            user.DisplayName,
            feedPostId,
            entity.Id,
        ...
        {
            await _feedInteractionNotificationService.RegisterMentionAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (likeId.HasValue)
        {
            if (hasLiked)
            {
                await _feedInteractionNotificationService.RegisterLikeAsync(
                    feedPost.PortalUserId,
                    portalUserId,
            ...
            {
                await _feedInteractionNotificationService.DeactivateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (shareId.HasValue)
        {
            if (hasShared)
            {
                await _feedInteractionNotificationService.RegisterShareAsync(
                    feedPost.PortalUserId,
                    portalUserId,
                    auditContext.ActorDisplayName,
                    feedPostId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (saveId.HasValue)
        {
            if (hasSaved)
            {
                await _feedInteractionNotificationService.RegisterSaveAsync(
                    feedPost.PortalUserId,
                    portalUserId,
                    auditContext.ActorDisplayName,
                    feedPostId,
    ...
    private async Task<IReadOnlyList<FeedItemDto>> BuildShareFeedItemsAsync(
        Guid? portalUserId,
        IReadOnlyDictionary<Guid, int> userPostLikeCounts,
        IReadOnlyDictionary<Guid, int> userPostShareCounts,
        IReadOnlyCollection<Guid> likedUserPostIds,
        IReadOnlyCollection<Guid> sharedUserPostIds,
        IReadOnlyCollection<Guid> savedUserPostIds,
        IReadOnlyDictionary<Guid, int> mediaCommentCounts,
        Dictionary<Guid, List<FeedPostCommentDto>> postComments,
        Dictionary<Guid, List<FeedPostCommentMentionDto>> postMentions,
        CancellationToken cancellationToken)
    {
        var shares = await _dbContext.FeedPostShares
            .AsNoTracking()
            .Include(item => item.PortalUser)
            .Include(item => item.FeedPost!)
                .ThenInclude(post => post.PortalUser)
            .Include(item => item.FeedPost!)
                .ThenInclude(post => post.Media)
            .Where(item => item.FeedPost != null && item.FeedPost.DeletedAtUtc == null)
            .OrderByDescending(item => item.CreatedAtUtc)
            .ToListAsync(cancellationToken);
        ...
        {
            var post = share.FeedPost!;
            var media = MapMediaItems(post.Media, mediaCommentCounts);
            var comments = postComments.GetValueOrDefault(post.Id) ?? [];
            var mentions = postMentions.GetValueOrDefault(post.Id) ?? [];

            return new FeedItemDto(
                share.Id,
                FeedItemSources.UserPostShare,
                null,
                post.PortalUser?.DisplayName ?? "Colaborador",
                post.PortalUserId,
                post.PortalUser?.Department ?? "Companhia",
                share.CreatedAtUtc,
                post.Text,
                mentions,
                null,
                null,
                media.FirstOrDefault()?.Url,
                userPostLikeCounts.GetValueOrDefault(post.Id),
                likedUserPostIds.Contains(post.Id),
                userPostShareCounts.GetValueOrDefault(post.Id),
                sharedUserPostIds.Contains(post.Id),
                savedUserPostIds.Contains(post.Id),
                media,
                comments.Count,
                comments,
                share.PortalUser?.DisplayName ?? "Colaborador",
                share.PortalUserId,
                share.CreatedAtUtc,
                post.Id);
        }).ToList();
